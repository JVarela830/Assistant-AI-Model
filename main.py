...
def type_of_search(query, model):
    prompt_decisor = f"""
    Requires internet search? (YES/NO)
    If it needs real-time, news, weather, or public world facts: YES.
    If it is personal (pets, family, greetings, chitchat): NO.
    
    Message: {query}
    Answer:"""
    
    fast_model = model.bind(
        stop=["\n", " "], 
        options={"temperature": 0.0, "num_predict": 5}
    )
    
    decision = fast_model.invoke(prompt_decisor).strip().upper()
    ...
    while True:
        ...
        if question == "q":
            ...
            if relevant_data:
                ...
                decision = type_of_search(question, model)

                if decision:
                    result = websearch.search_web(question)
                    context = result
